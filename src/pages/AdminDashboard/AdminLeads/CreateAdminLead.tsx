...
import { useCreateAdminLeadMutation } from '@/redux/features/admin/lead/createAdminLeadApi';
import PhoneInput, { isValidPhoneNumber } from 'react-phone-number-input';
import 'react-phone-number-input/style.css';
import { toast } from 'react-toastify';

const CreateAdminLead = () => {
    const navigate = useNavigate();
    const [createAdminLead, { isLoading }] = useCreateAdminLeadMutation();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await createAdminLead(leadData).unwrap();
            ...
            if (err.data) {
                ...
                if (err.data.client_name) backendErrors.client_name = Array.isArray(err.data.client_name) ? err.data.client_name[0] : err.data.client_name;
                if (err.data.email_address) backendErrors.email_address = Array.isArray(err.data.email_address) ? err.data.email_address[0] : err.data.email_address;
                if (err.data.property) backendErrors.property = Array.isArray(err.data.property) ? err.data.property[0] : err.data.property;
                if (err.data.sqft_range) backendErrors.sqft_range = Array.isArray(err.data.sqft_range) ? err.data.sqft_range[0] : err.data.sqft_range;
            ...
            <div className="bg-white border-b border-gray-200">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">Create New Lead (Admin)</h1>
...
export default CreateAdminLead;
