import { useAddVendorMutation } from '@/redux/features/admin/vendor-management/vendorManagementApi';
...
const AddVendorModal: React.FC<AddVendorModalProps> = ({
    ...
    const [addVendor, { isLoading, error: apiError }] = useAddVendorMutation();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (err) {
                toast.error(err?.data?.message || err?.data?.email?.[0] || 'Failed to add vendor.');
            } else {
            ...
            <div className="bg-white rounded-lg w-full max-w-lg mt-8 mb-8 shadow-xl">
                ...
                <div className="relative px-6 pt-6 pb-4">
                    ...
                    <p className="text-gray-600 text-sm leading-relaxed">
                        Add a new vendor to your platform. They will be able to list and manage properties.
                    </p>
                </div>
                ...
                {apiError && (
                    <div className="mx-6 mb-4 p-3 bg-red-50 border border-red-200 rounded-md flex items-center gap-2">
                        ...
                        <p className="text-red-600 text-sm">
                            {'data' in apiError ? (apiError.data as any)?.message || 'Failed to add vendor' : 'Failed to add vendor'}
                        </p>
                    </div>
                )}
                ...
                <div className="px-6 pb-6">
                    ...
                    <div className="mb-5">
                        ...
                        <input
                            ...
                            className={`w-full px-4 py-3 border rounded-md text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.email ? 'border-red-500' : 'border-gray-300'}`}
                            disabled={isLoading}
                        />
                        {errors.email && (
